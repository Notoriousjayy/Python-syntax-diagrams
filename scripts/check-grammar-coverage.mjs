 ...
 * 
 * Detects drift between diagram factories (pythonGrammar.ts) and EBNF definitions (ebnfDefinitions.ts).
...
// Extract rule names from pythonGrammar.ts by parsing rules.set() calls
...
function main() {
  ...
  const grammarPath = resolve(srcDir, 'pythonGrammar.ts');
  ...
  try {
    ...
    console.log(`📊 Found ${diagramRules.size} diagram rules in pythonGrammar.ts`);
  } catch (err) {
    console.error(`❌ Failed to read pythonGrammar.ts: ${err.message}`);
    process.exit(1);
  }
