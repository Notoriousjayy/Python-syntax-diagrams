// ebnfDefinitions.ts
//
// EBNF definitions for Python 3.14 PEG grammar rules.
// Each entry maps a rule name to its EBNF representation.
// Based on the official Python PEG grammar (Grammar/python.gram) from CPython.

import type { RuleName } from "./pythonGrammar";

/**
 * EBNF definitions keyed by rule name.
 * Format follows the Python PEG grammar notation:
 * - Strings with single quotes (') denote KEYWORDS
 * - Strings with double quotes (") denote SOFT KEYWORDS
 * - Upper case names (NAME) denote tokens
 * - e1 e2 means match e1, then match e2
 * - e1 | e2 means match e1 or e2
 * - [ e ] or e? means optionally match e
 * - e* means zero or more occurrences of e
 * - e+ means one or more occurrences of e
 * - s.e+ means one or more of e, separated by s
 */
export const EBNF_DEFINITIONS: Record<RuleName, string> = {
  // ===== STARTING RULES =====
  "file": `file:
    [statements] ENDMARKER`,

  "interactive": `interactive:
    statement_newline`,

  "eval": `eval:
    expressions NEWLINE* ENDMARKER`,

  "func_type": `func_type:
    '(' [type_expressions] ')' '->' expression NEWLINE* ENDMARKER`,

  // ===== GENERAL STATEMENTS =====
  "statements": `statements:
    statement+`,

  "statement": `statement:
    | compound_stmt
    | simple_stmts`,

  "single_compound_stmt": `single_compound_stmt:
    compound_stmt`,

  "statement_newline": `statement_newline:
    | single_compound_stmt NEWLINE
    | simple_stmts
    | NEWLINE
    | ENDMARKER`,

  "simple_stmts": `simple_stmts:
    | simple_stmt !';' NEWLINE
    | ';'.simple_stmt+ [';'] NEWLINE`,

  "simple_stmt": `simple_stmt:
    | assignment
    | type_alias
    | star_expressions
    | return_stmt
    | import_stmt
    | raise_stmt
    | pass_stmt
    | del_stmt
    | yield_stmt
    | assert_stmt
    | break_stmt
    | continue_stmt
    | global_stmt
    | nonlocal_stmt`,

  "compound_stmt": `compound_stmt:
    | function_def
    | if_stmt
    | class_def
    | with_stmt
    | for_stmt
    | try_stmt
    | while_stmt
    | match_stmt`,

  // ===== SIMPLE STATEMENTS =====
  "assignment": `assignment:
    | NAME ':' expression ['=' annotated_rhs]
    | ('(' single_target ')' | single_subscript_attribute_target) ':' expression ['=' annotated_rhs]
    | (star_targets '=')+ annotated_rhs !'=' [TYPE_COMMENT]
    | single_target augassign ~ annotated_rhs`,

  "annotated_rhs": `annotated_rhs:
    | yield_expr
    | star_expressions`,

  "augassign": `augassign:
    | '+=' | '-=' | '*=' | '@=' | '/=' | '%='
    | '&=' | '|=' | '^=' | '<<=' | '>>='
    | '**=' | '//='`,

  "return_stmt": `return_stmt:
    'return' [star_expressions]`,

  "raise_stmt": `raise_stmt:
    | 'raise' expression ['from' expression]
    | 'raise'`,

  "pass_stmt": `pass_stmt:
    'pass'`,

  "break_stmt": `break_stmt:
    'break'`,

  "continue_stmt": `continue_stmt:
    'continue'`,

  "global_stmt": `global_stmt:
    'global' ','.NAME+`,

  "nonlocal_stmt": `nonlocal_stmt:
    'nonlocal' ','.NAME+`,

  "del_stmt": `del_stmt:
    'del' del_targets &(';' | NEWLINE)`,

  "yield_stmt": `yield_stmt:
    yield_expr`,

  "assert_stmt": `assert_stmt:
    'assert' expression [',' expression]`,

  "import_stmt": `import_stmt:
    | import_name
    | import_from`,

  // ===== IMPORT STATEMENTS =====
  "import_name": `import_name:
    'import' dotted_as_names`,

  "import_from": `import_from:
    | 'from' ('.' | '...')* dotted_name 'import' import_from_targets
    | 'from' ('.' | '...')+ 'import' import_from_targets`,

  "import_from_targets": `import_from_targets:
    | '(' import_from_as_names [','] ')'
    | import_from_as_names !','
    | '*'`,

  "import_from_as_names": `import_from_as_names:
    ','.import_from_as_name+`,

  "import_from_as_name": `import_from_as_name:
    NAME ['as' NAME]`,

  "dotted_as_names": `dotted_as_names:
    ','.dotted_as_name+`,

  "dotted_as_name": `dotted_as_name:
    dotted_name ['as' NAME]`,

  "dotted_name": `dotted_name:
    | dotted_name '.' NAME
    | NAME`,

  // ===== COMPOUND STATEMENTS =====
  "block": `block:
    | NEWLINE INDENT statements DEDENT
    | simple_stmts`,

  "decorators": `decorators:
    ('@' named_expression NEWLINE)+`,

  "class_def": `class_def:
    | decorators class_def_raw
    | class_def_raw`,

  "class_def_raw": `class_def_raw:
    'class' NAME [type_params] ['(' [arguments] ')'] ':' block`,

  "function_def": `function_def:
    | decorators function_def_raw
    | function_def_raw`,

  "function_def_raw": `function_def_raw:
    | 'def' NAME [type_params] '(' [params] ')' ['->' expression] ':' [func_type_comment] block
    | 'async' 'def' NAME [type_params] '(' [params] ')' ['->' expression] ':' [func_type_comment] block`,

  // ===== FUNCTION PARAMETERS =====
  "params": `params:
    parameters`,

  "parameters": `parameters:
    | slash_no_default param_no_default* param_with_default* [star_etc]
    | slash_with_default param_with_default* [star_etc]
    | param_no_default+ param_with_default* [star_etc]
    | param_with_default+ [star_etc]
    | star_etc`,

  "slash_no_default": `slash_no_default:
    | param_no_default+ '/' ','
    | param_no_default+ '/' &')'`,

  "slash_with_default": `slash_with_default:
    | param_no_default* param_with_default+ '/' ','
    | param_no_default* param_with_default+ '/' &')'`,

  "star_etc": `star_etc:
    | '*' param_no_default param_maybe_default* [kwds]
    | '*' param_no_default_star_annotation param_maybe_default* [kwds]
    | '*' ',' param_maybe_default+ [kwds]
    | kwds`,

  "kwds": `kwds:
    '**' param_no_default`,

  "param_no_default": `param_no_default:
    | param ',' TYPE_COMMENT?
    | param TYPE_COMMENT? &')'`,

  "param_no_default_star_annotation": `param_no_default_star_annotation:
    | param_star_annotation ',' TYPE_COMMENT?
    | param_star_annotation TYPE_COMMENT? &')'`,

  "param_with_default": `param_with_default:
    | param default ',' TYPE_COMMENT?
    | param default TYPE_COMMENT? &')'`,

  "param_maybe_default": `param_maybe_default:
    | param default? ',' TYPE_COMMENT?
    | param default? TYPE_COMMENT? &')'`,

  "param": `param:
    NAME annotation?`,

  "param_star_annotation": `param_star_annotation:
    NAME star_annotation`,

  "annotation": `annotation:
    ':' expression`,

  "star_annotation": `star_annotation:
    ':' star_expression`,

  "default": `default:
    '=' expression`,

  // ===== CONTROL FLOW =====
  "if_stmt": `if_stmt:
    | 'if' named_expression ':' block elif_stmt
    | 'if' named_expression ':' block [else_block]`,

  "elif_stmt": `elif_stmt:
    | 'elif' named_expression ':' block elif_stmt
    | 'elif' named_expression ':' block [else_block]`,

  "else_block": `else_block:
    'else' ':' block`,

  "while_stmt": `while_stmt:
    'while' named_expression ':' block [else_block]`,

  "for_stmt": `for_stmt:
    | 'for' star_targets 'in' ~ star_expressions ':' [TYPE_COMMENT] block [else_block]
    | 'async' 'for' star_targets 'in' ~ star_expressions ':' [TYPE_COMMENT] block [else_block]`,

  "with_stmt": `with_stmt:
    | 'with' '(' ','.with_item+ ','? ')' ':' [TYPE_COMMENT] block
    | 'with' ','.with_item+ ':' [TYPE_COMMENT] block
    | 'async' 'with' '(' ','.with_item+ ','? ')' ':' block
    | 'async' 'with' ','.with_item+ ':' [TYPE_COMMENT] block`,

  "with_item": `with_item:
    | expression 'as' star_target &(',' | ')' | ':')
    | expression`,

  "try_stmt": `try_stmt:
    | 'try' ':' block finally_block
    | 'try' ':' block except_block+ [else_block] [finally_block]
    | 'try' ':' block except_star_block+ [else_block] [finally_block]`,

  "except_block": `except_block:
    | 'except' expression ':' block
    | 'except' expression 'as' NAME ':' block
    | 'except' expressions ':' block
    | 'except' ':' block`,

  "except_star_block": `except_star_block:
    | 'except' '*' expression ':' block
    | 'except' '*' expression 'as' NAME ':' block
    | 'except' '*' expressions ':' block`,

  "finally_block": `finally_block:
    'finally' ':' block`,

  // ===== PATTERN MATCHING =====
  "match_stmt": `match_stmt:
    "match" subject_expr ':' NEWLINE INDENT case_block+ DEDENT`,

  "subject_expr": `subject_expr:
    | star_named_expression ',' star_named_expressions?
    | named_expression`,

  "case_block": `case_block:
    "case" patterns guard? ':' block`,

  "guard": `guard:
    'if' named_expression`,

  "patterns": `patterns:
    | open_sequence_pattern
    | pattern`,

  "pattern": `pattern:
    | as_pattern
    | or_pattern`,

  "as_pattern": `as_pattern:
    or_pattern 'as' pattern_capture_target`,

  "or_pattern": `or_pattern:
    '|'.closed_pattern+`,

  "closed_pattern": `closed_pattern:
    | literal_pattern
    | capture_pattern
    | wildcard_pattern
    | value_pattern
    | group_pattern
    | sequence_pattern
    | mapping_pattern
    | class_pattern`,

  "literal_pattern": `literal_pattern:
    | signed_number !('+' | '-')
    | complex_number
    | strings
    | 'None'
    | 'True'
    | 'False'`,

  "literal_expr": `literal_expr:
    | signed_number !('+' | '-')
    | complex_number
    | strings
    | 'None'
    | 'True'
    | 'False'`,

  "complex_number": `complex_number:
    | signed_real_number '+' imaginary_number
    | signed_real_number '-' imaginary_number`,

  "signed_number": `signed_number:
    | NUMBER
    | '-' NUMBER`,

  "signed_real_number": `signed_real_number:
    | real_number
    | '-' real_number`,

  "real_number": `real_number:
    NUMBER`,

  "imaginary_number": `imaginary_number:
    NUMBER`,

  "capture_pattern": `capture_pattern:
    pattern_capture_target`,

  "pattern_capture_target": `pattern_capture_target:
    !"_" NAME !('.' | '(' | '=')`,

  "wildcard_pattern": `wildcard_pattern:
    "_"`,

  "value_pattern": `value_pattern:
    attr !('.' | '(' | '=')`,

  "attr": `attr:
    name_or_attr '.' NAME`,

  "name_or_attr": `name_or_attr:
    | attr
    | NAME`,

  "group_pattern": `group_pattern:
    '(' pattern ')'`,

  "sequence_pattern": `sequence_pattern:
    | '[' maybe_sequence_pattern? ']'
    | '(' open_sequence_pattern? ')'`,

  "open_sequence_pattern": `open_sequence_pattern:
    maybe_star_pattern ',' maybe_sequence_pattern?`,

  "maybe_sequence_pattern": `maybe_sequence_pattern:
    ','.maybe_star_pattern+ ','?`,

  "maybe_star_pattern": `maybe_star_pattern:
    | star_pattern
    | pattern`,

  "star_pattern": `star_pattern:
    | '*' pattern_capture_target
    | '*' wildcard_pattern`,

  "mapping_pattern": `mapping_pattern:
    | '{' '}'
    | '{' double_star_pattern ','? '}'
    | '{' items_pattern ',' double_star_pattern ','? '}'
    | '{' items_pattern ','? '}'`,

  "items_pattern": `items_pattern:
    ','.key_value_pattern+`,

  "key_value_pattern": `key_value_pattern:
    (literal_expr | attr) ':' pattern`,

  "double_star_pattern": `double_star_pattern:
    '**' pattern_capture_target`,

  "class_pattern": `class_pattern:
    | name_or_attr '(' ')'
    | name_or_attr '(' positional_patterns ','? ')'
    | name_or_attr '(' keyword_patterns ','? ')'
    | name_or_attr '(' positional_patterns ',' keyword_patterns ','? ')'`,

  "positional_patterns": `positional_patterns:
    ','.pattern+`,

  "keyword_patterns": `keyword_patterns:
    ','.keyword_pattern+`,

  "keyword_pattern": `keyword_pattern:
    NAME '=' pattern`,

  // ===== TYPE STATEMENTS =====
  "type_alias": `type_alias:
    "type" NAME [type_params] '=' expression`,

  "type_params": `type_params:
    '[' type_param_seq ']'`,

  "type_param_seq": `type_param_seq:
    ','.type_param+ [',']`,

  "type_param": `type_param:
    | NAME [type_param_bound] [type_param_default]
    | '*' NAME [type_param_starred_default]
    | '**' NAME [type_param_default]`,

  "type_param_bound": `type_param_bound:
    ':' expression`,

  "type_param_default": `type_param_default:
    '=' expression`,

  "type_param_starred_default": `type_param_starred_default:
    '=' star_expression`,

  // ===== EXPRESSIONS =====
  "expressions": `expressions:
    | expression (',' expression)+ [',']
    | expression ','
    | expression`,

  "expression": `expression:
    | disjunction 'if' disjunction 'else' expression
    | disjunction
    | lambdef`,

  "yield_expr": `yield_expr:
    | 'yield' 'from' expression
    | 'yield' [star_expressions]`,

  "star_expressions": `star_expressions:
    | star_expression (',' star_expression)+ [',']
    | star_expression ','
    | star_expression`,

  "star_expression": `star_expression:
    | '*' bitwise_or
    | expression`,

  "star_named_expressions": `star_named_expressions:
    ','.star_named_expression+ [',']`,

  "star_named_expression": `star_named_expression:
    | '*' bitwise_or
    | named_expression`,

  "assignment_expression": `assignment_expression:
    NAME ':=' ~ expression`,

  "named_expression": `named_expression:
    | assignment_expression
    | expression !':='`,

  "disjunction": `disjunction:
    | conjunction ('or' conjunction)+
    | conjunction`,

  "conjunction": `conjunction:
    | inversion ('and' inversion)+
    | inversion`,

  "inversion": `inversion:
    | 'not' inversion
    | comparison`,

  // ===== COMPARISON OPERATORS =====
  "comparison": `comparison:
    | bitwise_or compare_op_bitwise_or_pair+
    | bitwise_or`,

  "compare_op_bitwise_or_pair": `compare_op_bitwise_or_pair:
    | eq_bitwise_or
    | noteq_bitwise_or
    | lte_bitwise_or
    | lt_bitwise_or
    | gte_bitwise_or
    | gt_bitwise_or
    | notin_bitwise_or
    | in_bitwise_or
    | isnot_bitwise_or
    | is_bitwise_or`,

  "eq_bitwise_or": `eq_bitwise_or:
    '==' bitwise_or`,

  "noteq_bitwise_or": `noteq_bitwise_or:
    '!=' bitwise_or`,

  "lte_bitwise_or": `lte_bitwise_or:
    '<=' bitwise_or`,

  "lt_bitwise_or": `lt_bitwise_or:
    '<' bitwise_or`,

  "gte_bitwise_or": `gte_bitwise_or:
    '>=' bitwise_or`,

  "gt_bitwise_or": `gt_bitwise_or:
    '>' bitwise_or`,

  "notin_bitwise_or": `notin_bitwise_or:
    'not' 'in' bitwise_or`,

  "in_bitwise_or": `in_bitwise_or:
    'in' bitwise_or`,

  "isnot_bitwise_or": `isnot_bitwise_or:
    'is' 'not' bitwise_or`,

  "is_bitwise_or": `is_bitwise_or:
    'is' bitwise_or`,

  // ===== BITWISE OPERATORS =====
  "bitwise_or": `bitwise_or:
    | bitwise_or '|' bitwise_xor
    | bitwise_xor`,

  "bitwise_xor": `bitwise_xor:
    | bitwise_xor '^' bitwise_and
    | bitwise_and`,

  "bitwise_and": `bitwise_and:
    | bitwise_and '&' shift_expr
    | shift_expr`,

  "shift_expr": `shift_expr:
    | shift_expr '<<' sum
    | shift_expr '>>' sum
    | sum`,

  // ===== ARITHMETIC OPERATORS =====
  "sum": `sum:
    | sum '+' term
    | sum '-' term
    | term`,

  "term": `term:
    | term '*' factor
    | term '/' factor
    | term '//' factor
    | term '%' factor
    | term '@' factor
    | factor`,

  "factor": `factor:
    | '+' factor
    | '-' factor
    | '~' factor
    | power`,

  "power": `power:
    | await_primary '**' factor
    | await_primary`,

  // ===== PRIMARY ELEMENTS =====
  "await_primary": `await_primary:
    | 'await' primary
    | primary`,

  "primary": `primary:
    | primary '.' NAME
    | primary genexp
    | primary '(' [arguments] ')'
    | primary '[' slices ']'
    | atom`,

  "slices": `slices:
    | slice !','
    | ','.(slice | starred_expression)+ [',']`,

  "slice": `slice:
    | [expression] ':' [expression] [':' [expression]]
    | named_expression`,

  "atom": `atom:
    | NAME
    | 'True'
    | 'False'
    | 'None'
    | strings
    | NUMBER
    | (tuple | group | genexp)
    | (list | listcomp)
    | (dict | set | dictcomp | setcomp)
    | '...'`,

  "group": `group:
    '(' (yield_expr | named_expression) ')'`,

  // ===== LAMBDA FUNCTIONS =====
  "lambdef": `lambdef:
    'lambda' [lambda_params] ':' expression`,

  "lambda_params": `lambda_params:
    lambda_parameters`,

  "lambda_parameters": `lambda_parameters:
    | lambda_slash_no_default lambda_param_no_default* lambda_param_with_default* [lambda_star_etc]
    | lambda_slash_with_default lambda_param_with_default* [lambda_star_etc]
    | lambda_param_no_default+ lambda_param_with_default* [lambda_star_etc]
    | lambda_param_with_default+ [lambda_star_etc]
    | lambda_star_etc`,

  "lambda_slash_no_default": `lambda_slash_no_default:
    | lambda_param_no_default+ '/' ','
    | lambda_param_no_default+ '/' &':'`,

  "lambda_slash_with_default": `lambda_slash_with_default:
    | lambda_param_no_default* lambda_param_with_default+ '/' ','
    | lambda_param_no_default* lambda_param_with_default+ '/' &':'`,

  "lambda_star_etc": `lambda_star_etc:
    | '*' lambda_param_no_default lambda_param_maybe_default* [lambda_kwds]
    | '*' ',' lambda_param_maybe_default+ [lambda_kwds]
    | lambda_kwds`,

  "lambda_kwds": `lambda_kwds:
    '**' lambda_param_no_default`,

  "lambda_param_no_default": `lambda_param_no_default:
    | lambda_param ','
    | lambda_param &':'`,

  "lambda_param_with_default": `lambda_param_with_default:
    | lambda_param default ','
    | lambda_param default &':'`,

  "lambda_param_maybe_default": `lambda_param_maybe_default:
    | lambda_param default? ','
    | lambda_param default? &':'`,

  "lambda_param": `lambda_param:
    NAME`,

  // ===== LITERALS =====
  "fstring_middle": `fstring_middle:
    | fstring_replacement_field
    | FSTRING_MIDDLE`,

  "fstring_replacement_field": `fstring_replacement_field:
    '{' annotated_rhs '='? [fstring_conversion] [fstring_full_format_spec] '}'`,

  "fstring_conversion": `fstring_conversion:
    "!" NAME`,

  "fstring_full_format_spec": `fstring_full_format_spec:
    ':' fstring_format_spec*`,

  "fstring_format_spec": `fstring_format_spec:
    | FSTRING_MIDDLE
    | fstring_replacement_field`,

  "fstring": `fstring:
    FSTRING_START fstring_middle* FSTRING_END`,

  "tstring_format_spec_replacement_field": `tstring_format_spec_replacement_field:
    '{' annotated_rhs '='? [fstring_conversion] [tstring_full_format_spec] '}'`,

  "tstring_format_spec": `tstring_format_spec:
    | TSTRING_MIDDLE
    | tstring_format_spec_replacement_field`,

  "tstring_full_format_spec": `tstring_full_format_spec:
    ':' tstring_format_spec*`,

  "tstring_replacement_field": `tstring_replacement_field:
    '{' annotated_rhs '='? [fstring_conversion] [tstring_full_format_spec] '}'`,

  "tstring_middle": `tstring_middle:
    | tstring_replacement_field
    | TSTRING_MIDDLE`,

  "tstring": `tstring:
    TSTRING_START tstring_middle* TSTRING_END`,

  "string": `string:
    STRING`,

  "strings": `strings:
    | (fstring | string)+
    | tstring+`,

  // ===== COLLECTIONS =====
  "list": `list:
    '[' [star_named_expressions] ']'`,

  "tuple": `tuple:
    '(' [star_named_expression ',' [star_named_expressions]] ')'`,

  "set": `set:
    '{' star_named_expressions '}'`,

  "dict": `dict:
    '{' [double_starred_kvpairs] '}'`,

  "double_starred_kvpairs": `double_starred_kvpairs:
    ','.double_starred_kvpair+ [',']`,

  "double_starred_kvpair": `double_starred_kvpair:
    | '**' bitwise_or
    | kvpair`,

  "kvpair": `kvpair:
    expression ':' expression`,

  // ===== COMPREHENSIONS & GENERATORS =====
  "for_if_clauses": `for_if_clauses:
    for_if_clause+`,

  "for_if_clause": `for_if_clause:
    | 'async' 'for' star_targets 'in' ~ disjunction ('if' disjunction)*
    | 'for' star_targets 'in' ~ disjunction ('if' disjunction)*`,

  "listcomp": `listcomp:
    '[' named_expression for_if_clauses ']'`,

  "setcomp": `setcomp:
    '{' named_expression for_if_clauses '}'`,

  "genexp": `genexp:
    '(' (assignment_expression | expression !':=') for_if_clauses ')'`,

  "dictcomp": `dictcomp:
    '{' kvpair for_if_clauses '}'`,

  // ===== FUNCTION CALL ARGUMENTS =====
  "arguments": `arguments:
    args [','] &')'`,

  "args": `args:
    | ','.(starred_expression | (assignment_expression | expression !':=') !'=')+ [',' kwargs]
    | kwargs`,

  "kwargs": `kwargs:
    | ','.kwarg_or_starred+ ',' ','.kwarg_or_double_starred+
    | ','.kwarg_or_starred+
    | ','.kwarg_or_double_starred+`,

  "starred_expression": `starred_expression:
    '*' expression`,

  "kwarg_or_starred": `kwarg_or_starred:
    | NAME '=' expression
    | starred_expression`,

  "kwarg_or_double_starred": `kwarg_or_double_starred:
    | NAME '=' expression
    | '**' expression`,

  // ===== ASSIGNMENT TARGETS =====
  "star_targets": `star_targets:
    | star_target !','
    | star_target (',' star_target)* [',']`,

  "star_targets_list_seq": `star_targets_list_seq:
    ','.star_target+ [',']`,

  "star_targets_tuple_seq": `star_targets_tuple_seq:
    | star_target (',' star_target)+ [',']
    | star_target ','`,

  "star_target": `star_target:
    | '*' (!'*' star_target)
    | target_with_star_atom`,

  "target_with_star_atom": `target_with_star_atom:
    | t_primary '.' NAME !t_lookahead
    | t_primary '[' slices ']' !t_lookahead
    | star_atom`,

  "star_atom": `star_atom:
    | NAME
    | '(' target_with_star_atom ')'
    | '(' [star_targets_tuple_seq] ')'
    | '[' [star_targets_list_seq] ']'`,

  "single_target": `single_target:
    | single_subscript_attribute_target
    | NAME
    | '(' single_target ')'`,

  "single_subscript_attribute_target": `single_subscript_attribute_target:
    | t_primary '.' NAME !t_lookahead
    | t_primary '[' slices ']' !t_lookahead`,

  "t_primary": `t_primary:
    | t_primary '.' NAME &t_lookahead
    | t_primary '[' slices ']' &t_lookahead
    | t_primary genexp &t_lookahead
    | t_primary '(' [arguments] ')' &t_lookahead
    | atom &t_lookahead`,

  "t_lookahead": `t_lookahead:
    '(' | '[' | '.'`,

  "del_targets": `del_targets:
    ','.del_target+ [',']`,

  "del_target": `del_target:
    | t_primary '.' NAME !t_lookahead
    | t_primary '[' slices ']' !t_lookahead
    | del_t_atom`,

  "del_t_atom": `del_t_atom:
    | NAME
    | '(' del_target ')'
    | '(' [del_targets] ')'
    | '[' [del_targets] ']'`,

  // ===== TYPING ELEMENTS =====
  "type_expressions": `type_expressions:
    | ','.expression+ ',' '*' expression ',' '**' expression
    | ','.expression+ ',' '*' expression
    | ','.expression+ ',' '**' expression
    | '*' expression ',' '**' expression
    | '*' expression
    | '**' expression
    | ','.expression+`,

  "func_type_comment": `func_type_comment:
    | NEWLINE TYPE_COMMENT &(NEWLINE INDENT)
    | TYPE_COMMENT`,
};

/**
 * Gets the EBNF definition for a given rule name.
 * Returns undefined if not found.
 */
export function getEbnfDefinition(name: RuleName): string | undefined {
  return EBNF_DEFINITIONS[name];
}

/**
 * Get all EBNF rule names for grammar coverage checking.
 */
export function getEbnfRuleNames(): string[] {
  return Object.keys(EBNF_DEFINITIONS);
}
